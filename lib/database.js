...
Database.prototype.addModel = function(alias,schema) {
	db.once('open', function callback() {
		var modelName = mongoose.model(alias, schema);

		return modelName;
	});
};

Database.prototype.addDocument = function(content) {
	db.once('open', function callback() {
...
db.once('open', function callback() {
	...
	Database.prototype.addSchema = function(content) {
		...
		var schemaName = new Schema(content);
